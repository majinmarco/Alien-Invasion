...
class AlienInvasion:
	...
	def _check_play_button(self, mouse_pos):
		...
		if button_clicked and not self.stats.game_active:
			...
			self.settings.initlialize_dynamic_settings()
	...
	def _ship_hit(self):
		...
		if self.stats.ships_left > 0:
			...
			with open('files/high_score.txt', 'r+') as f:
				if self.stats.score > int(f.read()):
					f.truncate(0)
					f.write(str(self.stats.score))
			pygame.mouse.set_visible(True)

	def _update_stars(self):
		self._check_star_row_edges()
		self.stars.update()
	...
	def _create_star(self, star_number, row_number):
		...
		star.rect.y = star.y +(2*star.y + row_number)
		self.stars.add(star)			
	...
	def _check_star_row_edges(self):
		for star in self.stars.sprites():
			if star.check_edges():
				self._new_star_row()
				break

	def _new_star_row(self):
		star = Star(self)
		star_width, star_height = star.rect.size
		...
		for star_number in range(number_stars_x):
			random_number = randint(0, self.settings.star_density)
			if random_number==1:
				self._create_star(star_number, 0)

	def _check_fleet_edges(self):
		"""Respons appropriately if any aliens have reached an edge."""
